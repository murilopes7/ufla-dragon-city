 ...
 * Membros da Equipe:
 *   - Felipe - 20252022
 *		► Criação de funções principais e de auxiliares
 *   - Murilo Santos Lopes Carvalho - 202520216
 *		► Criação dos menus NotCurses, de funções principais e de auxiliares
 *   - Pedro Augusto Castro Oliveira - 202520222
 *		► Criação do README e de funções principais
 *
 * Instituição: UFLA
			...
			if (key == NCKEY_UP) {
				...
				if (selecionado == 2) {
					...
					if (selecionado == 0) {
						...
						// === VERIFICAR ID INICIAL ===
						while (id_inicio < 1 || id_inicio > tamanho ||
							   cin.fail()) {
							...
							if (cin.fail() || id_inicio < 1 ||
								id_inicio > tamanho) {
								cout << "\nValor inválido! O índice "
										"inicial deve estar entre 1 e "
									 << tamanho << ".\n\n";
								limparEntrada();
								id_inicio = 0;
							}
						...
						// === VERIFICAR ID FINAL ===
						while (id_fim < id_inicio || id_fim > tamanho ||
							   cin.fail()) {
							cout << "Digite o índice final (" << id_inicio
								 << " a " << tamanho << "): ";
							cin >> id_fim;
							if (cin.fail() || id_fim < id_inicio ||
								id_fim > tamanho) {
								cout << "\nValor inválido! O índice "
										"final deve ser entre "
									 << id_inicio << " e " << tamanho
									 << ", e não pode ser menor que o índice "
										"inicial.\n\n";
								limparEntrada();
								id_fim = 0;
							}
		...
		if (ni.evtype != NCTYPE_RELEASE) {
			if (key == NCKEY_UP)
				...
				if (selecionado == 0) {
					...
					// Verifica se o usuário está digitando um número natural
					// não nulo.
					qtd = verifyIfNumber();
...
bool carregar_dados_csv(Dragon*& dragao, int& tamanho, int& capacidade,
						const string& arquivo_string) {
	ifstream arquivo(arquivo_string);
	if (!arquivo.is_open()) {
